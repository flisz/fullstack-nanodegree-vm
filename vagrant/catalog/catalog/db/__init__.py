...
# Example Import: from catalog.db import get_db
# Example Usage: db = get_db()
def get_db():
    return DB()
...
class DB(object):
    ...
    def add_category(self, data, commit=True, session=None):
        if session is None:
            session = get_session()
        required = dict()
    ...
    def get_categories_where(self, data, session=None):
        if session is None:
            session = get_session()
    ...
    def update_category(self, data, commit=True, session=None):
        if session is None:
            session = get_session()
        # Requirements:
        required = dict()
    ...
    def delete_category(self, category, commit=False, session=None):
        if session is None:
            session = get_session()
    ...
    def delete_categories_where(self, data, commit=True, session=None):
        if session is None:
            session = get_session()
        categories = get_categories_where(self, data, session=session)
        for category in categories:
    ...
    def get_category_items(self, category, session=None):
        if session is None:
            session = get_session()
    ...
    def get_category_and_items(self, data, session=None):
        if session is None:
            session = get_session()
        required = dict()
        required['category_id'] = data.get('category_id')
    ...
    def add_item(self, data, commit=True, session=None):
        if session is None:
            session = get_session()
        required = dict()
        required["name"] = data.get("name")
    ...
    def get_items_where(self, data, session=None):
        if session is None:
            session = get_session()
        print("GETTING:\titems:\n\twith criteria:{}".format(data))
        # Filter By Criteria:
    ...
    def update_item(self, data, commit=True, session=None):
        if session is None:
            session = get_session()
        required = dict()
    ...
    def delete_item(self, item, commit=False, session=None):
        if session is None:
            session = get_session()
        print("DELETING:\tcategory:\t:\t{}".format(category))
        category_items = get_category_items(category)
